...
const MasterCourseSinglePage = () => {
	...
	useEffect(() => {
		...
		const fetchData = async () => {
			...
			try {
				...
				console.error('マスター講座お知らせの取得に失敗しました:', error);
				setNotFound(true);
			} finally {
				setLoading(false);
			}
	...
	const handleCopyLink = async () => {
		const url = `${window.location.origin}/master-course/${noticeId}`;
		try {
...
export default MasterCourseSinglePage;
