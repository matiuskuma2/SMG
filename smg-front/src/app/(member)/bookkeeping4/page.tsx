...
const Bookkeeping4ListContent = () => {
	const [hasAccess, setHasAccess] = useState<boolean | null>(null);

	// ユーザーが4期簿記講座にアクセス可能かチェック
	useEffect(() => {
		const checkGroupAccess = async () => {
			try {
				...
				// mst_groupテーブルから「【第4期】簿記講座」「運営」「講師」グループのIDを取得
				const { data: groupData, error: groupError } = await supabase
					.from('mst_group')
					.select('group_id, title')
					.in('title', ['【第4期】簿記講座', '運営', '講師'])
					.is('deleted_at', null);
				...
				if (!groupData || groupData.length === 0) {
					console.error(
						'必要なグループ（【第4期】簿記講座、運営、講師）が見つかりません',
					);
					setHasAccess(false);
					return;
				}
		...
		async (
			...
			try {
				...
				if (userId) {
					...
					// ユーザーが基本グループ（【第4期】簿記講座、運営、講師）に所属しているかチェック
	...
	return (
		<ListContent
			...
			bannerAlt="4期簿記講座"
			basePath="/bookkeeping4"
			placeholderText="簿記講座名で検索"
			emptyText="4期簿記講座が見つかりませんでした。"
...
const Bookkeeping4ListPaginated = () => {
	return (
		<Suspense fallback={<div>Loading...</div>}>
			<Bookkeeping4ListContent />
		</Suspense>
	);
};

export default Bookkeeping4ListPaginated;
