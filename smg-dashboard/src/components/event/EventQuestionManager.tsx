...
import type { EventQuestion, EventQuestionFormType } from './types';

interface EventQuestionManagerProps {
  eventId: string | null;
  isEditing: boolean;
  initialQuestions?: EventQuestionFormType[];
  onQuestionsChange?: (questions: EventQuestionFormType[]) => void;
}

export function EventQuestionManager({
  eventId,
  initialQuestions = [],
  onQuestionsChange,
}: EventQuestionManagerProps) {
  const [questions, setQuestions] = useState<EventQuestion[]>([]);
  const [loading, setLoading] = useState(false);
  const [showForm, setShowForm] = useState(false);
  const [editingQuestion, setEditingQuestion] = useState<EventQuestion | null>(
    null,
  );
  const [formData, setFormData] = useState<EventQuestionFormType>({
    title: '',
    question_type: 'text',
    is_required: false,
    display_order: 0,
  ...
  const [localQuestions, setLocalQuestions] =
    useState<EventQuestionFormType[]>(initialQuestions);
  ...
  const fetchQuestions = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`/api/event-questions?event_id=${eventId}`);
      if (!response.ok) {
  ...
  const resetForm = () => {
    setFormData({
      ...
      question_type: 'text',
      is_required: false,
      display_order: eventId ? questions.length : localQuestions.length,
      options: [],
    });
    setOptionIds([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (eventId) {
      ...
      try {
        ...
        console.log('API呼び出し:', { method, body });

        const response = await fetch('/api/event-questions', {
          method,
          headers: {
      ...
      // 新規作成時：ローカルで管理
      console.log('新規作成モード: ローカルで管理します');
      if (editingQuestion) {
        // ローカル質問の編集
        console.log('ローカル質問の編集');
        const updatedQuestions = localQuestions.map((q, index) =>
          index === editingQuestion.display_order ? formData : q,
        );
        console.log('更新後のlocalQuestions:', updatedQuestions);
        setLocalQuestions(updatedQuestions);
        onQuestionsChange?.(updatedQuestions);
      } else {
        // ローカル質問の追加
        console.log('ローカル質問の追加');
        const newQuestion = {
          ...formData,
    ...
    if (eventId) {
      ...
      try {
        ...
        const response = await fetch(
          `/api/event-questions?question_id=${questionIdOrIndex}`,
          {
            method: 'DELETE',
          },
        );
  ...
  // 編集開始
  const handleEdit = (
    question: EventQuestion | EventQuestionFormType,
    index?: number,
  ) => {
    if (eventId) {
      // 編集時：通常の処理
      const q = question as EventQuestion;
      ...
      // 新規作成時：ローカル質問の編集
      const q = question as EventQuestionFormType;
      setEditingQuestion({
        ...q,
        question_id: '',
        event_id: '',
  ...
  return (
    <div className={containerStyle}>
      ...
      <div className={questionsListStyle}>
        {(() => {
          const displayQuestions = eventId
            ? questions
            : localQuestions.map(
                (q, index) =>
                  ({
