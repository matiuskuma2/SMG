...
export default function TabEditPage() {
    ...
    const fetchTab = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const tab = data.tab;
          setDisplayName(tab.display_name);
          setLinkType(tab.link_type);
          setLinkValue(tab.link_value);
          setDisplayOrder(tab.display_order);
          setStatus(tab.status);
          setIsVisibleToAll(tab.is_visible_to_all);
          setSelectedGroupIds(
            (tab.visible_groups || []).map((g: any) => g.group_id),
          );
        } else {
          alert('タブが見つかりません');
          router.push('/tablist');
  ...
  // link_type 変更時の自動リンク値設定
  const handleLinkTypeChange = (newType: string) => {
    setLinkType(newType);
    if (AUTO_LINK_VALUES[newType]) {
      setLinkValue(AUTO_LINK_VALUES[newType]);
    }
  };
