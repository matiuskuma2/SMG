...
export default function ArchiveCreatePage() {
  ...
  const [defaultEventTypeId, setDefaultEventTypeId] = useState<string>('');
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // デフォルトのevent_type_idを取得（なしアーカイブ用）
        ...
        if (!eventTypeError && eventTypeData && eventTypeData.length > 0) {
          setDefaultEventTypeId(eventTypeData[0].event_type_id);
        }
  ...
  const handleSubmit = async (data: ArchiveFormData) => {
    try {
      ...
      if (!isEditing && !defaultEventTypeId) {
        alert(
          'システムエラー: イベントタイプが取得できませんでした。管理者にお問い合わせください。',
        );
        return;
      }
      ...
      if (isEditing && archiveId) {
        ...
        savedArchiveId = updatedData.archive_id;
      } else {
        // 新規作成処理
        const insertData = {
          title: data.title,
      ...
      // 表示グループ保存処理
      await saveVisibleGroups(savedArchiveId, data.visible_group_ids || []);
