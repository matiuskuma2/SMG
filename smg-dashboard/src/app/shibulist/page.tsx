...
// 支部カテゴリのdescription値
const CATEGORY_TYPE = 'shibu';

function ShibuListContent() {
  ...
  // 支部カテゴリIDリスト
  const [shibuCategoryIds, setShibuCategoryIds] = useState<string[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const shibuCatIds = (categoryData || []).map((c) => c.category_id);
        setShibuCategoryIds(shibuCatIds);
        ...
        if (shibuCatIds.length > 0) {
          query = query.in('category_id', shibuCatIds);
        } else {
          // 支部カテゴリがない場合は空の結果
          setNotices([]);
          setIsLoading(false);
  ...
  const handleEdit = (noticeId: string) => {
    const editUrl = buildEditPageUrl(`/shibu/edit/${noticeId}`, searchParams);
    router.push(editUrl);
  };
