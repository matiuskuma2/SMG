...
export default function MasterCourseCreatePage() {
  ...
  const handleSubmit = async (data: NoticeFormData, isDraft = false) => {
    ...
    try {
      ...
      if (error) {
        console.error('Error creating master course notice:', error);
        setError('マスター講座投稿の作成に失敗しました');
        return;
      }
